...
static app_pc dis(void *drcontext, app_pc pc, app_pc display_pc) {
...
#define STUB_STRUCT_MAX_SIZE                                                   \
  (sizeof(tracedump_stub_data_t) + SEPARATE_STUB_MAX_SIZE)
#define STUBIDX(stubs, i)                                                      \
  ((tracedump_stub_data_t *)(((byte *)stubs) + (i)*STUB_STRUCT_MAX_SIZE))

static void read_data(file_t f, void *drcontext) {
  byte sbuf[BUF_SIZE];
  byte *dbuf = NULL;
  ssize_t read;
  int dlen = 0, i;
  ...
  if (fhdr.version != _USES_DR_VERSION_) {
    dr_fprintf(STDERR,
               "Error: file version %d does not match tool version %d\n",
  ...
  while (1) {
    ...
    for (cur_stub = 0; cur_stub < hdrs.num_exits; cur_stub++) {
      bool separate = STUBIDX(stubs, cur_stub)->stub_pc < hdrs.cache_start_pc ||
                      STUBIDX(stubs, cur_stub)->stub_pc >=
                          hdrs.cache_start_pc + hdrs.code_size;
